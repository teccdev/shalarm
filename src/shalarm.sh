#!/bin/bash
################################################################################
#   shalarm.sh      |   version 2.0     |   FreeBSD License   |   2026.05.04
#   Original by James Hendrie [2013-2018]
#   Maintained by Austin Wilson (aka Teccin) [2026-present]
#   github.com/teccdev/shalarm
#   <teccin@proton.me>
################################################################################
...
function control_c {
	...
	##  ringing, then snooze.
		...
		if [[ $ringTheAlarm -eq 1 ]]; then
			echo -e "Snoozing for $snooze seconds\n"
			start_snoozing
		else
			echo -e "Exiting\n"
			exit 0
...
while true; do
	...
	if [[ $ringTheAlarm -eq 0 ]]; then
		...
		leastSecond=$(((10#$alarmHour - $currentHour) * 60 * 60 + (\
			...
			10#$alarmSecond - $currentSecond) - 1))
		...
		echo -ne "\r   ${leastHour}${leastMinute}${leastSecond}left to sleep"
